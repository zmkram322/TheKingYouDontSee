...
editHistory:
  ...
  - date: '2026-08-05'
    changes: 'Amended FR86 and its matching Core Mechanical Model prose during the node-based rebuild in tkyds-game/game/. Protection of the survival/interpersonal set now comes from **composition** — those categories are present in every actor by construction — rather than from an explicit mark whose effect was to skip the action''s own eligibility predicate. The mark was built, found to be redundant whenever the gate returns true, and actively harmful otherwise: it silently discards a predicate an author wrote, which fails more dangerously than the ungated action it guarded against. The guarantee FR86 exists for ("nothing may be pruned from this set by identity, only outbid by weight") is unchanged and is now kept at the layer that actually controls it. No FR renumbered; no other FR affected. Coding conventions for the rebuild live in the repo''s CLAUDE.md, not in this document.'
  - date: '2026-08-03'
...
- **A protected universal set sits in every actor's repertoire, always.** Survival responses and direct interpersonal actions (eat, flee, confront, beg, steal, hold ground) are **composed into every actor by construction** and compete in their scoring pass regardless of role. Their eligibility is evaluated normally — protection is about what an actor *carries*, not about skipping a gate. A flag that overrode an action's own predicate was built and removed: it meant an author could write a gate that silently did nothing, a worse failure than the ungated action it was meant to prevent. **Protection is by category, never by instance:** a starving man must always be able to attempt `eat`; he is not entitled to have every inn in the region scored. This is the same guarantee that lets a sufficiently-held actor hold his post while starving: nothing may be pruned from this set by identity, only by weight.
...
- FR86: A protected set of survival and direct interpersonal action **categories** (eat, flee, confront, beg, steal, hold ground) is present in every actor's repertoire **by construction** — composed into the base actor, so no role, faction, possession, or position can remove them. Their eligibility predicates are evaluated normally, like any other action's: protection means an actor always *carries* the capability and can only be outbid, never that a gate is skipped. An override that discards an authored predicate is explicitly rejected — it makes a written gate silently inert, which fails more dangerously than the omission it was guarding against. Protection covers the **category**, never a particular instance of it: narrowing which providers an actor considers is legitimate, leaving the category with no instance at all is not.
