class_name FarmingSlotActivity
extends Activity

# Transient — contributes deltas to its FarmingDayActivity parent and is then
# discarded. Class-level constants are this activity-class's calibration;
# sibling classes (BlacksmithSlotActivity, etc.) declare their own.
# v0 calibration: 1 grain/slot, no XP/calorie wiring yet.

const W_ATH := 0.7
const W_CHA := 0.0
const W_INT := 0.3
const BASE_OUTPUT := 1.0           # v0: matches Phase 2 (4 grain/day per worker)
...
func _init() -> void:
	activity_type = &"FarmingSlotActivity"

func on_close() -> bool:
	if parent_activity_ref == null:
		push_error("FarmingSlotActivity must have a parent (FarmingDayActivity).")
		return false
	var day := parent_activity_ref as FarmingDayActivity
	if day == null:
		push_error("FarmingSlotActivity parent must be a FarmingDayActivity.")
		return false

	var aptitude_factor: float = _aptitude_factor()
	var slot_output: float = BASE_OUTPUT * (1.0 + _variance(aptitude_factor))
	...
	day.output_produced_today += slot_output
	day.skill_xp_earned += slot_xp
